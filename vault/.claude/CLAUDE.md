...
## Главный принцип: TickTick = эталон, Notion = CRM-зеркало

**TickTick** — рабочий инструмент Максима, источник правды о задачах.
**Notion PM Backlog** — CRM-система, которая зеркалит TickTick.

### Правила, обязательные для любой функции затрагивающей задачи:
1. **Количество совпадает**: сколько активных задач в TickTick на этой неделе — столько же в Notion "Спринт неделя"
2. **Закрытие синхронно**: закрыл задачу в TickTick → в Notion автоматически переходит в "Выполнено"
3. **Создание синхронно**: добавил задачу в TickTick на эту неделю → появляется в Notion "Спринт неделя"
4. **Цель**: скорость и чистота, не накопление хаоса

### Принцип двух алгоритмов:
Технические реализации для TickTick и Notion могут быть разными (разные API, разные поля), но логически они описывают одно и то же действие. Всегда спрашивай: «Если я делаю X в TickTick — что должно произойти в Notion?» и реализуй обе стороны.
...
### Перенос задачи в TickTick

Когда пользователь говорит «перенеси задачу», «поставь на [время]», «сдвинь на [дату]» для СУЩЕСТВУЮЩЕЙ задачи — это перенос, а не создание новой.

**Правило:** НИКОГДА не создавай новую задачу через `add_task` если задача уже существует в TickTick. Только `update_task`.

**Порядок действий:**
1. Найди задачу через `search_task` по названию — получи `id` и `projectId`
2. Используй `update_task` с новыми `startDate` и `dueDate` в формате `YYYY-MM-DDTHH:MM:SS+10:00`
3. Если пользователь явно назвал дату и время — используй именно их, даже если это 13:00 (обед не блокирует явно выбранное пользователем время)

**Триггерные фразы:** «перенеси», «сдвинь», «поставь на», «хочу сегодня в», «перенеси на завтра», «поставь в план»

**Обновление Notion при переносе:**
После обновления даты в TickTick — найди эту задачу в PM Backlog по названию и обнови статус:
- Новая дата попадает в текущую неделю (пн–пт) → статус «Спринт неделя»
- Новая дата за пределами текущей недели → статус «Бэклог»

Текущая неделя: `WEEK_START` = ближайший понедельник ≤ сегодня, `WEEK_END` = WEEK_START + 4 дня.
