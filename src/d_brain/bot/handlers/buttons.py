...
def _build_plan_prompt(day_label: str, date_str: str, date_iso: str, occupied_slots: str = "", backlog_count: int | None = None) -> str:
    ...
    backlog_hint = (
        f"⚠️ ОБЯЗАТЕЛЬНО ДО НАЧАЛА: в Notion PM Backlog сейчас {backlog_count} активных задач. "
        "Фраза «бэклог пуст», «бэклог чист», «беклог пуст» или любой аналог — ЗАПРЕЩЕНА КАТЕГОРИЧЕСКИ. "
        "Это фактическая ошибка: задачи есть. Если ни одна не подходит — объясни почему (статус, приоритет, день уже полон).\n\n"
        if backlog_count is not None else
        "⚠️ ЗАПРЕЩЕНО говорить «бэклог пуст/чист» — это всегда фактическая ошибка. "
        "Если задач на день нет — объясни причину.\n\n"
    )
